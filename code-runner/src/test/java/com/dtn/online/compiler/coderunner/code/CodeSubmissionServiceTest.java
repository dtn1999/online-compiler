...
class CodeSubmissionServiceTest {
    ...
    void runCode_bad_request() {
        ...
        Mockito.when( codeSubmissionService.runCode(codeSubmissionDTO, ExecutionConstraints.builder().build()))
                .thenReturn(ApiResponse.builder().build());
    ...
    void runCode_good_request_but_bad_code() {
        ...
        mockMvc.perform(
                ...
                .andExpect( result -> {
                    ...
                    Assertions.assertNotNull(apiResponse.getData());
                    // confirm that the code run without error
                    ExecutionResult executionResult = (ExecutionResult) apiResponse.getData();
                    Assertions.assertEquals(ExecutionVerdict.TIME_LIMIT_EXCEPTION, executionResult.getVerdict());
                });
    }
